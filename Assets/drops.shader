Shader "drops"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex, _GrapTexture;
            float4 _MainTex_ST;
            ...
            float3 Layer(float2 UV, float t)
            {
                ...
                fogTrail *= S(.5, y, gv.y);
                trail *= fogTrail;
                fogTrail *= S(.05, 0.04, abs(dropPos.x));
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                drops += Layer(i.uv * 1.25 + 6.55, t);
                drops += Layer(i.uv * 1.35 + 2.15, t);
                drops += Layer(i.uv * 1.57 - 6.55, t);
                ...
                for (float i = 0;i < numSamples;i++)
                {
                    ...
                    float d = frac(sin((i + 1) * 562.) * 6232.);
                    d = sqrt(d);
                    offs *= d;
                    col += tex2D(_GrapTexture, projUV +offs);
                    a++;
                }
                col /= numSamples;
